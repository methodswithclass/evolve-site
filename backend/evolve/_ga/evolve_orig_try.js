	...
	var generation = function (params) {
		...
		var initializePop = function () {
			...
			if (params && params.pop) {
				...
				i = 0;
				while (i < self.total) {
					self.pop[i] = new individual({gen:0, input:input});
					i++;
				}
			}
		...
		this.getstepdata = function (stepdata) {
			...
			stepdata = self.pop[org].getstepdata(stepdata);
		...
		this.hardStop = function () {
			...
			console.log("generation hard stop", org);

			active = false;
			clearInterval(runtimer);
			runtimer = null;
			if (self.pop[org]) self.pop[org].hardStop();
			initializePop();
		}
