app.factory("doc.data", function () {
	...
	},
	{
		...
		content:[
		{
			...
			text:[
			[
				"The Trash demo is a very interesting case. It is classic machine learning. A robot is tasked with cleaning trash in an grid. It has minimal sensation of the cells adjacent to it, a wall, trash, or empty. It needs a plan in order to know what to do for each situation it finds itself in. Perfomance can be easily measured by the more trash picked up and the less walls run into. The plan is designed so that no matter what arrangment of trash, for the same plan, the robot will yield a similar performance. The algorithm is the perfect tool in this case to find the optimal plan.",
				"The Trash demo is great because it shows the power of the evolutionary algorithm so plainly. After only a few generations (100 or so) the robot will usually end up running into walls or staying in place (wasting it's alotted steps,, gaining no points). But when you get into generations numbering five hundred or a thousand, all of a sudden the robot won't run into a singe wall, it will pick up trash where it sees it, and it will even move toward trash in order to pick it up. Very smart.",
				"There is a phenomenon that happens at about the thousand generation mark that is truly remarkable and spooky. It appears that the robot learns patience. If it finds itself sitting over trash with trash adjacent on either side, it will not immediately pick up the trash below it, like you would think. It will follow the trash in one direction until it reaches the end of the line, and then zip down the entire line. This ends up being much more effient than picking up that original trash in the middle immediately like us humans would have done. The algorithm figured that out. Spooky."
			]
			]
		},
		// {
