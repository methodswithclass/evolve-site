app.factory('trash-sim', [
  '$http',
  'utility',
  'api.service',
  'input.service',
  function ($http, u, api, $input) {
          ...
          score: {
            ...
            change:
              after.success == 'success'
                ? after.action.points.success
                : after.action.points.fail,
            total: _score,
          },
          move: {
            num: $i,
    ...
    var performStep = function (_input) {
      //   console.log('simulate ' + i, totalActions);

      if (_input.step) u.toggle('hide', 'step');

      if (active && _input.i <= totalActions) {
        api.simulate.trash(
          { name: name, i: _input.i, session: _input.session },
          function (res) {
    ...
    var refreshenv = function (complete) {
      ...
      api.refreshEnvironment(function (res) {
        ...
        makeBlocks(environment, 'refresh');
